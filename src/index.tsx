...
const camelCaseToKebabCase = (input: string): string =>
    input.replace(/[A-Z]+(?![a-z])|[A-Z]/g, ($, ofs) => (ofs ? "-" : "") + $.toLowerCase())

export default function withPropellors<T>(Component: React.ComponentType<T>) {
        ...
        <Component
            {
                ...
                .filter(([ key ]) => 
                    !cssProperties.all.includes(camelCaseToKebabCase(key))
                    ))
            }
            css={{
                ...props,
